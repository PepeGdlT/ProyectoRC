...
public class NFControllerLogicDir {
	...
	protected void testCommunicationWithDirectory(String directoryHostname) throws IOException {
		...
		if (directoryConnector.testSendAndReceive()) {
			...
			System.err.println("[testMode] Test FALLADO!");
		}
	}
	...
	protected boolean doLogin(String directoryHostname, String nickname) {
		...
		try {
			...
			return this.directoryConnector.logIntoDirectory(nickname);
		} catch (IOException e) {
			System.err.println("* Error: Unable to connect to the directory");
		}
	...
	protected boolean getAndPrintUserList() {
		 ...
		 */
		assert(this.directoryConnector.getUserList() != null);
		System.out.println("Userlist:   " + DirMessage.toStringArray(this.directoryConnector.getUserList()));
		return true;
	}
	...
	protected boolean getAndPrintFileList() {
		...
		FileInfo[] archivos = this.directoryConnector.getFileList();

		FileInfo.printToSysout(archivos);
		result = true;
	...
	private InetSocketAddress lookupServerAddrByUsername(String nickname) {
		...
		if (serverAddr == null) {
			if(nickname != null ) {
				System.err.println("* Error: Unable to get the server address from - " + nickname );
				
			}
			else {
				System.err.println("* Error: Unable to get the server address ");
			}
		}
	...
	public InetSocketAddress getServerAddress(String serverNicknameOrSocketAddr) {
		...
		} else if(serverNicknameOrSocketAddr.equals("localhost")) {
			String ip = "127.0.0.1";
			String port = "6868";
			fserverAddr = new InetSocketAddress(ip, Integer.parseInt(port));
		}
	...
	public boolean getAndPrintServersNicknamesSharingThisFile(String fileHashSubstring) {
		...
		assert(this.directoryConnector.getServerNicknamesSharingThisFile(fileHashSubstring) != null);

		System.out.println("Usuarios con el fichero buscado:\n" + DirMessage.toStringArray(this.directoryConnector.getServerNicknamesSharingThisFile(fileHashSubstring)));
	...
	public LinkedList<InetSocketAddress> getServerAddressesSharingThisFile(String downloadTargetFileHash) {
		LinkedList<InetSocketAddress> serverAddressList = new LinkedList<InetSocketAddress>();
		/*
		 * TODO: Comunicarse con el directorio (a través del directoryConnector) para
		 * preguntar por aquellos servidores que están sirviendo un determinado fichero,
		 ...
		 * 
		 */
		assert(this.directoryConnector.getServerNicknamesSharingThisFile(downloadTargetFileHash) != null);
		String[] nicks = this.directoryConnector.getServerNicknamesSharingThisFile(downloadTargetFileHash);

		for (String nick : nicks) {
			try {
				serverAddressList.add(getServerAddress(nick));
			} catch (NullPointerException e) {
				System.err.println("* Error: Unable to get the server address list");
				return null;
			}
		}
	...
	public boolean unregisterFileServer() {
		...
		result = this.directoryConnector.unregisterServerPort();
